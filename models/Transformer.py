...
class PatchEmbed(nn.Module):
    """
    A compatible Patch Embedding:
    - supports both (patch_size) or (kernel_size/stride/padding)
    - supports in_chans / in_channels
    - supports embed_dim / embed_dims
    - supports norm_cfg={'type': 'BN' or 'LN'}
    """
    def __init__(self,
                 img_size=224,
                 patch_size=16,
                 ...
                 in_channels=None,
                 embed_dims=None,
                 # optional conv args (MMSeg style)
                 kernel_size=None,
                 stride=None,
                 padding=None,
                 bias=True,
        ...
        if embed_dim is None:
            raise ValueError("PatchEmbed: must provide embed_dim or embed_dims")

        # ---- conv params ----
        # allow either (patch_size) or explicit (kernel_size/stride/padding)
        if kernel_size is None:
            kernel_size = patch_size
        if stride is None:
            stride = patch_size
        if padding is None:
            # 常见做法：若需要下采样保持中心，可用 stride//2；调用处已给 padding 就按调用处
            padding = 0
        ...
        # ---- norm ----
        # 优先 norm_cfg，其次 norm_layer（向后兼容）
        self.bn = None
        self.ln = None
        if norm_cfg is not None and isinstance(norm_cfg, dict):
            norm_type = norm_cfg.get('type', '').upper()
            if norm_type in ['BN', 'SYNCBN']:
                self.bn = nn.BatchNorm2d(embed_dim)
            elif norm_type == 'LN':
                # LN 作用在展平后的 [N, HW, C] 的 C 维
                self.ln = nn.LayerNorm(embed_dim)
        elif norm_layer is not None:
            # 若传入的是一个可调用的归一化层构造器（很少用到），尽力兼容
            # 这里默认作为 2D BN 使用；如果你确实想要 LN，请用 norm_cfg={'type':'LN'}
    ...
    def forward(self, x):
        ...
        if self.bn is not None:
            x = self.bn(x)
        x = nchw_to_nlc(x)               # [N, H'*W', embed_dim]
        if self.ln is not None:
            x = self.ln(x)               # LN on last dim
        return x


def resize(input, size=None, scale_factor=None, mode='bilinear', align_corners=False, warning=False):
    """
    替代 mmseg.ops.resize，直接用 F.interpolate
    """
