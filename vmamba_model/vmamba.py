...
class SS2D(nn.Module, mamba_init):
    ...
    def __initv2__(
        ...
        **kwargs,    
        ...
        self.out_proj = Linear(512 , d_model , bias=bias, **factory_kwargs)
        self.dropout = nn.Dropout(dropout) if dropout > 0. else nn.Identity()
    ...
    # only used to run previous version
    def forwardv0(self, x: torch.Tensor, SelectiveScan = SelectiveScanCore, seq=False, force_fp32=True, **kwargs):
        print("###forwardv0###debug###")
        print("###1x.shape###debug###",x.shape)
        x = self.in_proj(x)
        print("###2x.shape###debug###",x.shape)
        x, z = x.chunk(2, dim=1) # (b, h, w, d)
        print("###3x.shape###debug###",x.shape)
        print("###1z.shape###debug###",z.shape)
        z = self.act(z)
        print("###2z.shape###debug###",z.shape)
        #x = x.permute(0, 3, 1, 2).contiguous()
        print("###4x.shape###debug###",x.shape)
        x = self.conv2d(x) # (b, d, h, w)
        print("###5x.shape###debug###",x.shape)
        x = self.act(x)
        print("###6x.shape###debug###",x.shape)
        
        def selective_scan(u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=True, nrows=1):
            return SelectiveScan.apply(u, delta, A, B, C, D, delta_bias, delta_softplus, nrows, False)

        B, D, H, W = x.shape
        print("###B, D, H, W###debug###",B, D, H, W)
        D, N = self.A_logs.shape
        print("###D, N###debug###",D, N)
        K, D, R = self.dt_projs_weight.shape
        print("###K, D, R###debug###",K, D, R)
        L = H * W
        print("###L###debug###",L)

        x_hwwh = torch.stack([x.view(B, -1, L), torch.transpose(x, dim0=2, dim1=3).contiguous().view(B, -1, L)], dim=1).view(B, 2, -1, L)
        print("###x_hwwh###debug###",x_hwwh.shape)
        xs = torch.cat([x_hwwh, torch.flip(x_hwwh, dims=[-1])], dim=1) # (b, k, d, l)
        print("###xs###debug###",xs.shape)
        ...
        print("###1y.shape###debug###",y.shape)
        
        y = y.transpose(dim0=1, dim1=2).contiguous() # (B, L, C)
        print("###2y.shape###debug###",y.shape)
        y = self.out_norm(y).view(B, H, W, -1)
        y = y.permute(0, 3, 1, 2)
        print("###3y.shape###debug###",y.shape)
        print("###z.shape###debug###",z.shape)
        y = y * z
        print("###4y.shape###debug###",y.shape)
    ...
    def forwardv2(self, x: torch.Tensor, **kwargs):
        ...
        if not self.disable_z:
            x, z = x.chunk(2, dim=(1)) # (b, h, w, d)# if self.channel_first else -1
        ...
        print(f"###y.shape: {y.shape}")
        #y = self.out_proj(y)
        #print(f"###self.out_proj(y).shape: {y.shape}")
        
        out = self.dropout(self.out_proj(y))
        print(f"###self.dropout.shape: {out.shape}")
